# Copyright (c) brdav Truong. All rights reserved.
#
# This source code is licensed under the license found in the
# LICENSE file in https://github.com/brdav/cma.
#
...
model_urls = {
    ...
    "megadepth": "./pretrained_weights/uawarpc_megadepth.ckpt"
}
...
class LocalFeatureCorrelationLayer(nn.Module):
    ...
    def forward(self, feature_source, feature_target):
        ...
        corr = self.local_correlation(feature_target, feature_source, patch_size=self.patch_size)
        corr = corr.view(b, self.patch_size * self.patch_size, feature_target.shape[2], feature_target.shape[3])
...
class GlobalFeatureCorrelationLayer(nn.Module):
    ...
    def forward(self, feature_source, feature_target):
        ...
        corr = self.compute_global_correlation(feature_source, feature_target)

        if self.cyclic_consistency:
            # to add on top of the correlation ! (already included in NC-Net)
            corr4d = self.mutual_matching(
                corr.view(
                    b,
                    feature_source.shape[2],
                    feature_source.shape[3],
                    feature_target.shape[2],
                    feature_target.shape[3],
                ).unsqueeze(1)
            )
    ...
    def compute_global_correlation(feature_source, feature_target, shape="3D", put_W_first_in_channel_dimension=False):
        if shape == "3D":
            ...
            if put_W_first_in_channel_dimension:
                ...
                correlation_tensor = (
                    feature_mul.view(b, h_target, w_target, w_source * h_source).transpose(2, 3).transpose(1, 2)
                )
                ...
                # perform matrix mult.
                feature_mul = torch.bmm(
                    feature_target, feature_source
                )  # shape (b,h_target*w_target, h_source*w_source)
                correlation_tensor = (
                    feature_mul.view(b, h_target, w_target, h_source * w_source).transpose(2, 3).transpose(1, 2)
                )
...
class OpticalFlowEstimatorResidualConnection(nn.Module):
    ...
    def __init__(self, in_channels, out_channels=2, batch_norm=True, output_x=False, extra_bias="auto"):
        ...
        self.conv_0 = ConvBNReLU(
            in_channels, 128, kernel_size=3, norm_layer=norm_layer, activation_layer=None, bias=extra_bias
        )
        self.conv0_skip = ConvBNReLU(128, 96, kernel_size=1, norm_layer=norm_layer, activation_layer=None)
        self.conv_1 = ConvBNReLU(
            128, 128, kernel_size=3, norm_layer=norm_layer, activation_layer=activation_layer, bias=extra_bias
        )
        self.conv_2 = ConvBNReLU(128, 96, kernel_size=3, norm_layer=norm_layer, activation_layer=None, bias=extra_bias)
        self.conv2_skip = ConvBNReLU(96, 32, kernel_size=1, norm_layer=norm_layer, activation_layer=None)
        self.conv_3 = ConvBNReLU(
            96, 64, kernel_size=3, norm_layer=norm_layer, activation_layer=activation_layer, bias=extra_bias
        )
        self.conv_4 = ConvBNReLU(64, 32, kernel_size=3, norm_layer=norm_layer, activation_layer=None, bias=extra_bias)
        self.predict_mapping = nn.Conv2d(32, out_channels, kernel_size=3, padding=1, bias=True)
...
class UncertaintyModule(nn.Module):
    ...
    def __init__(
        ...
        if self.search_size == 9:
            ...
            self.predict_uncertainty = nn.Conv2d(16, 6, kernel_size=3, stride=1, padding=0, bias=True)
        elif search_size == 16:
            self.conv_0 = ConvBNReLU(
                in_channels,
                32,
        ...
        self.predict_uncertainty_final = nn.Conv2d(16, out_channels, kernel_size=3, stride=1, padding=1, bias=True)

    def forward(self, corr, feat, up_previous_uncertainty=None, up_previous_flow=None, logits_corr=None):
        # x shape is b, s*s, h, w
        b, _, h, w = corr.size()
        x = corr.permute(0, 2, 3, 1).contiguous().view(b * h * w, self.search_size, self.search_size).unsqueeze(1)
        # x is now shape b*h*w, 1, s, s

        if logits_corr is not None:
            x_logits = (
                logits_corr.permute(0, 2, 3, 1)
                .contiguous()
                .view(b * h * w, self.search_size, self.search_size)
                .unsqueeze(1)
            )
...
class UAWarpCHead(BaseHead):
    def __init__(
        ...
        self.global_corr = GlobalFeatureCorrelationLayer(cyclic_consistency=True)
        self.local_corr = LocalFeatureCorrelationLayer(patch_size=9)

        # level 4, 16x16, global correlation
        nd = 16 * 16
        od = nd
        self.decoder4 = OpticalFlowEstimatorResidualConnection(in_channels=od, batch_norm=batch_norm, output_x=True)
        ...
        self.decoder3 = OpticalFlowEstimatorResidualConnection(in_channels=od, batch_norm=batch_norm, output_x=True)

        self.refinement_at_adaptive_res = refinement_at_adaptive_res
        if self.refinement_at_adaptive_res:
            self.refinement_module_adaptive = RefinementModule(32, batch_norm=batch_norm)
        ...
        if self.estimate_uncertainty:
            od += 1  # input also the upsampled log_var of previous resolution
        self.decoder2 = OpticalFlowEstimatorResidualConnection(in_channels=od, batch_norm=batch_norm, output_x=True)

        self.reduce = nn.Conv2d(32, nbr_upfeat_channels, kernel_size=1, bias=True)

        od = nd + 2 + nbr_upfeat_channels
        if self.estimate_uncertainty:
            od += 1  # input also the upsampled log_var of previous resolution
        self.decoder1 = OpticalFlowEstimatorResidualConnection(in_channels=od, batch_norm=batch_norm, output_x=True)

        self.refinement_at_finest_level = refinement_at_finest_level
        if self.refinement_at_finest_level:
            self.refinement_module_finest = RefinementModule(32, batch_norm=batch_norm)

        if self.estimate_uncertainty:
            self.estimate_uncertainty_components4 = UncertaintyModule(in_channels=1, search_size=16)
            self.estimate_uncertainty_components3 = UncertaintyModule(
                in_channels=1, search_size=9, feed_in_previous=True
            )
            self.estimate_uncertainty_components2 = UncertaintyModule(
                in_channels=1, search_size=9, feed_in_previous=True
            )
            self.estimate_uncertainty_components1 = UncertaintyModule(
                in_channels=1, search_size=9, feed_in_previous=True
            )
    ...
    def forward(self, trg, src, trg_256, src_256, out_size, **kwargs):
        ...
        if self.estimate_uncertainty:
            uncert_components4_256 = self.estimate_uncertainty_components4(corr4, x4)
            # scale uncert values to 256
            assert w_256 / float(w_4) == h_256 / float(h_4)
            uncert_components4_256[:, 0, :, :] += 2 * math.log(w_256 / float(w_4))

        # level 3: 32x32
        h_3, w_3 = c13.shape[-2:]
        assert (h_3, w_3) == (32, 32), (h_3, w_3)
        up_flow4 = nn.functional.interpolate(input=flow4_256, size=(h_3, w_3), mode="bilinear", align_corners=False)
        if self.estimate_uncertainty:
        ...
        if self.estimate_uncertainty:
            inp_flow_dec3 = torch.cat((corr3, up_flow4, up_uncert_components4), 1)
        else:
        ...
        if self.estimate_uncertainty:
            uncert_components3 = self.estimate_uncertainty_components3(corr3, x3, up_uncert_components4, up_flow4)
        ...
        if self.estimate_uncertainty:
            ...
            uncert_components3[:, 0, :, :] += 2 * math.log(diag_original / float(diag_256))
        ...
        if self.iterative_refinement and not self.training:
            ...
            nbr_extra_layers = max(0, int(round(math.log(R / minimum_ratio) / math.log(2))))
            if nbr_extra_layers > 0:
                for n in range(nbr_extra_layers):
                    ...
                    up_flow3 = nn.functional.interpolate(
                        input=flow3, size=(h_this, w_this), mode="bilinear", align_corners=False
                    )
                    if self.estimate_uncertainty:
                        up_uncert_components3 = nn.functional.interpolate(
                            input=uncert_components3, size=(h_this, w_this), mode="bilinear", align_corners=False
                        )
                    c23_bis = nn.functional.interpolate(c22, size=(h_this, w_this), mode="area")
                    c13_bis = nn.functional.interpolate(c12, size=(h_this, w_this), mode="area")
                    warp3 = warp(c23_bis, up_flow3 * ratio)
                    corr3 = self.local_corr(warp3, c13_bis)
                    if self.estimate_uncertainty:
                        inp_flow_dec3 = torch.cat((corr3, up_flow3, up_uncert_components3), 1)
                    else:
        ...
        h_2, w_2 = c12.shape[-2:]
        up_flow3 = nn.functional.interpolate(input=flow3, size=(h_2, w_2), mode="bilinear", align_corners=False)
        if self.estimate_uncertainty:
            up_uncert_components3 = nn.functional.interpolate(
        ...
        if self.estimate_uncertainty:
            inp_flow_dec2 = torch.cat((corr2, up_flow3, up_uncert_components3), 1)
        else:
        ...
        if self.estimate_uncertainty:
            uncert_components2 = self.estimate_uncertainty_components2(corr2, x2, up_uncert_components3, up_flow3)

        # level 1: 1/4 of original resolution
        h_1, w_1 = c11.shape[-2:]
        up_flow2 = nn.functional.interpolate(input=flow2, size=(h_1, w_1), mode="bilinear", align_corners=False)
        if self.estimate_uncertainty:
            up_uncert_components2 = nn.functional.interpolate(
                input=uncert_components2, size=(h_1, w_1), mode="bilinear", align_corners=False
            )

        up_feat2 = nn.functional.interpolate(input=x2, size=(h_1, w_1), mode="bilinear", align_corners=False)
        ...
        if self.estimate_uncertainty:
            inp_flow_dec1 = torch.cat((corr1, up_flow2, up_feat2, up_uncert_components2), 1)
        else:
        ...
        if self.estimate_uncertainty:
            uncert_components1 = self.estimate_uncertainty_components1(corr1, x, up_uncert_components2, up_flow2)

            # APPROXIMATION FOR NON-SQUARE IMAGES --> use the diagonal
            uncert_components4 = uncert_components4_256.clone()
            uncert_components4[:, 0, :, :] += 2 * math.log(diag_original / float(diag_256))

            return (
                (flow4, uncert_components4),
                (flow3, uncert_components3),
                (flow2, uncert_components2),
                (flow1, uncert_components1),
            )
    ...
    def unnormalise_and_convert_mapping_to_flow(map, output_channel_first=True):
        ...
        xx = torch.arange(0, W, dtype=mapping.dtype, device=mapping.device).view(1, -1).repeat(H, 1)
        yy = torch.arange(0, H, dtype=mapping.dtype, device=mapping.device).view(-1, 1).repeat(1, W)
    ...
    def load_weights(self, pretrain_path):
        ...
        if os.path.exists(pretrain_path):
            ...
            checkpoint = torch.load(
                os.path.join(os.environ.get("TORCH_HOME", ""), "hub", pretrain_path),
